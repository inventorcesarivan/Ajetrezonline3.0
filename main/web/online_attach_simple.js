...
// Ajetrez Online — Attacher for simple (injects button + status pill and wires Supabase)
(async () => {
  const simple = 'simple';
  ...
  btn.id = 'btn-online-' + simple;
  btn.className = 'btn small';
  btn.textContent = 'Online';
  btn.title = 'Jugar online (' + simple + ')';

  const pill = document.createElement('div');
  pill.id = 'online-' + simple + '-status';
  ...
  const setStatus = (t) => { pill.textContent = t; pill.title = t; console.log('[online-'+simple+']', t); };
